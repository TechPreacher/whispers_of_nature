class LandscapeAnimation {
    ...
    private hills: { x: number; height: number; speed: number; }[] = [];
    private clouds: { x: number; y: number; size: number; speed: number; }[] = [];
    private leaves: { x: number; y: number; size: number; rotation: number; speed: number; }[] = [];
    private baseHue: number = Math.random() * 360;
